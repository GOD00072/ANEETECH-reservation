...
function isOverlappingBooking(newBooking) {
  ...
  for (let booking of bookings) {
    if (booking.technician === technician) {
      ...
      if (
        (newStart >= existingStart - 60000 && newStart <= existingEnd + 60000) ||
        (newEnd >= existingStart - 60000 && newEnd <= existingEnd + 60000) ||
        (newStart <= existingStart - 60000 && newEnd >= existingEnd + 60000)
      ) {
        return true;
      }
    }
...
app.get('/checkAvailability', (req, res) => {
  ...
  const response = availableTechnicians.map(tech => {
    ...
    const isAvailable = !techBookings.some(b => {
      ...
      const selectedEnd = selectedStart + 60000;

      return (
        (selectedStart >= bookingStart - 60000 && selectedStart <= bookingEnd + 60000) ||
        (selectedEnd >= bookingStart - 60000 && selectedEnd <= bookingEnd + 60000) ||
        (selectedStart <= bookingStart - 60000 && selectedEnd >= bookingEnd + 60000)
      );
    });
